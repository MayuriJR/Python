...
class YananSpider(scrapy.Spider):
    ...
    name = 'sz'
    start_urls = ['http://www.weather.com.cn/weather/101190401.shtml']
    def parse(self, response):
        standSet=[]
        stand_csv = read_csv_file("/home/mayuri/python/weather_log/dataLogCsv/suzhou.csv")
        for i in stand_csv:
           standSet.append(i[0])
        ...
        for i in range(len(time0)): 
            ...
            if data[1][-1] !="l" and data [1][-1] !="\"":
                if data[0] not in standSet:
                    writerow_csv_file("/home/mayuri/python/weather_log/dataLogCsv/suzhou.csv",data)
                else:
                    print("%s exists"%data)
            else:
                print("%s is an invalid data"%data)
        print("*"*70)
        print("suzhou weather log successfully include 24 hours before%s"%print_time)
        print("*" * 70)
